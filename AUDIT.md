...
- **Status:** Partially addressed — code fix implemented, re-baseline outstanding. Both static servers now resolve every response `Content-Type` through the shared extension map in `scripts/utils/mime-types.mjs` (`scripts/qa-a11y.mjs`, `scripts/check-links.mjs`), which covers `.html`, `.css`, `.js`, `.mjs`, `.json`, `.webmanifest`, `.xml`, `.txt`, `.svg`, `.woff2`, `.png`, `.jpg`, `.jpeg`, `.webp`, `.avif` and `.ico`, uses `charset=utf-8` for text-based types, and keeps `application/octet-stream` only as the fallback for unknown extensions. Verified over real HTTP against this repository's files: `css/style.css` → `text/css; charset=utf-8`, `js/script.js` and `js/modules/ui-core.js` → `text/javascript; charset=utf-8`, `manifest.webmanifest` → `application/manifest+json; charset=utf-8`, `assets/fonts/*.woff2` → `font/woff2`, unknown extension → `application/octet-stream`. `npm run check:links` was re-run after the change and still reports only the three pre-existing P1-01 failures, confirming no behavioural change there.
- **Outstanding:** The corrected axe baseline required by the verification criteria has **not** been recorded. `npm run qa:a11y` cannot run in the current checkout — `node_modules` is absent, so `playwright` and `axe-core` fail to load and the script exits before starting its server. The browser-level claims in this finding (module script blocked, `window.SC` undefined, stylesheet not applied) therefore remain as originally measured, and the post-fix rendering has not yet been observed. After `npm install`, run `npm run qa:a11y` once, confirm the stylesheet is applied and `window.SC` is defined, and append the resulting `serious`/`critical` violation list here, marking each entry as pre-existing or newly surfaced.
